...
import { FaBars, FaTimes, FaSignOutAlt } from 'react-icons/fa'; // Import hamburger, close, and logout icons
    ...
    >
      ...
      <div className="md:hidden">
        <button onClick={toggleMenu} className="focus:outline-none">
          {isMenuOpen ? (
            <FaTimes className="text-indigo-600 text-2xl" /> // Close icon when menu is open
          ) : (
            <FaBars className="text-indigo-600 text-2xl" /> // Hamburger icon when menu is closed
          )}
        </button>
      </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-md rounded-b-md p-4 w-full">
          {isLoggedIn && (
            <>
              <Link
                href="/summarize"
